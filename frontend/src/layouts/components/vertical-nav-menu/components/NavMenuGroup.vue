...
.app-navigation-menu {
  ...
  @include theme(v-list-group__header) using ($material) {
    &.v-list-item--active {
      background-color: rgba(map-deep-get($material, 'text', 'secondary'), map-deep-get($material, 'states', 'selected'));
    }
  }
...
@include theme(vertical-nav-menu-group) using ($material) {
  &.vertical-nav-menu-group-active > .v-list-group__header {
    background-color: rgba(darkred, map-deep-get($material, 'states', 'selected'));
  }
}
